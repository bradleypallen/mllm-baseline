#!/usr/bin/env python3
"""
Tier 2 Enhanced Two-Tower - CONFIG D

Testing optimal head/batch combination:
- 4 attention heads (between original 3 and Config A's 6)
- Batch size 96 (scaling up from proven 64)
- Learning rate 0.0008 with cosine annealing
- All other successful tunings maintained

Hypothesis: Sweet spot around 4 heads + larger batches = better performance
"""
...
def train_epoch_config_d(model, train_loader, optimizer, criterion, device, hard_miner=None):
    """Training epoch with Config D optimizations (4 heads, batch 96)"""
    model.train()
    total_loss = 0
    num_batches = 0
...
def evaluate_model_config_d(model, val_loader, device):
    """Fast evaluation"""
    model.eval()
...
def run_tier2_config_d():
    """Run Tier 2 evaluation with Config D hyperparameters (4 heads, batch 96)"""
    print("="*80)
    print("TIER 2 ENHANCED TWO-TOWER - CONFIG D")
    print("="*80)
    print("Config D Optimizations:")
    print("  - 4 attention heads (sweet spot between 3 and 6)")
    print("  - Batch size: 96 (scaling up from proven 64)")
    ...
    print()
    print("Hypothesis: Optimal head count + larger batches → Better performance")
    print()
    ...
    print(f"Expected: Larger batches + optimal heads for better performance")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(unique_queries)):
        ...
        # Create model with Config D architecture (4 heads)
        model = TwoTowerModel(
            num_llms=num_llms,
        ...
        for epoch in range(epochs):
            ...
            train_loss = train_epoch_config_d(model, train_loader, optimizer, criterion, device, active_miner)
            ndcg_10, ndcg_5, mrr = evaluate_model_config_d(model, val_loader, device)
    ...
    print("\n" + "="*80)
    print("FINAL RESULTS - CONFIG D (4 HEADS, BATCH 96)")
    print("="*80)
    ...
    config_a_best = 0.4278  # Config A (6 heads)
    improvement_vs_original = np.mean(ndcg_10_scores) - original_best
    improvement_vs_config_a = np.mean(ndcg_10_scores) - config_a_best
    
    print(f"\nComparison Analysis:")
    print(f"  Original (3 heads, batch 32): {original_best:.4f} nDCG@10")
    print(f"  Config A (6 heads, batch 64): {config_a_best:.4f} nDCG@10")
    print(f"  Config D (4 heads, batch 96): {np.mean(ndcg_10_scores):.4f} nDCG@10")
    print(f"  vs Original: {improvement_vs_original:+.4f}")
    print(f"  vs Config A: {improvement_vs_config_a:+.4f}")
    
    # Prepare standardized results
    results = {
        'evaluation_info': {
            'timestamp': datetime.now().isoformat(),
            'pipeline': 'Tier2 CPU Optimized (Config D)',
            'description': 'Enhanced Two-Tower with Config D: 4 attention heads, batch size 96',
    ...
    # Save results
    save_standardized_results(results, 'tier2_cpu_optimized_config_d', '../../data/results/')
    print("\n✓ Results saved to data/results/tier2_cpu_optimized_config_d_results.json")
    
    print("\n" + "="*80)
    print("Config D (4 heads, batch 96) evaluation complete!")
    print("="*80)
