...
class PretrainedTwoTowerModel(nn.Module):
    ...
    def __init__(self, num_llms=1131, embedding_dim=128, num_heads=4, dropout=0.2,
        ...
        self.llm_embedding = nn.Embedding(num_llms, 256)  # Doubled from 128
        self.llm_tower = nn.Sequential(
            nn.Linear(256, 512),  # Adapted to new 256D embedding size
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(512, 256),
            nn.ReLU(),
...
def run_tier2_config_j():
    """Run Tier 2 evaluation with Config I (Config G + wider LLM embeddings)"""
    print("=" * 80)
    print("TIER 2 ENHANCED TWO-TOWER - CONFIG J")
    print("=" * 80)
    print("Config J - EVEN WIDER LLM EMBEDDINGS + EXTENDED TRAINING:")
    print("  - Base: Config I architecture")
    print("  - LLM embedding dimension: 128 → 256 (doubled again)")
    print("  - LLM tower: 256 → 512 → 256 → 64 (even wider)")
    ...
    print()
    print("Hypothesis: Maximum embedding capacity + longer training = peak performance")
    print()
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(unique_queries)):
        ...
        patience = 15  # Increased from 5 for longer convergence
        patience_counter = 0
    ...
    results = {
        'evaluation_info': {
            ...
            'pipeline': 'Tier2 Config J (Even Wider LLM Embeddings + Extended Training)',
            'description': 'Config I + doubled LLM embeddings (128 → 256D) + extended training (35 epochs, patience=15)',
    ...
    # Save results
    save_standardized_results(results, 'tier2_config_j', '../../data/results/')
    print("\n✓ Results saved to data/results/tier2_config_j_results.json")
    
    print("\n" + "=" * 80)
    print("Config J (256D LLM embeddings + extended training) evaluation complete!")
    print("=" * 80)
