#!/usr/bin/env python3
"""
Tier 2 Enhanced Two-Tower - CONFIG A

Next iteration of hyperparameter tuning:
- 6 attention heads (vs 4)
- Batch size 64 (vs 48) 
- Learning rate 0.0008 with cosine annealing
- All other successful tunings maintained

Testing to see if we can push closer to consistent 0.46+ performance.
"""
...
def train_epoch_config_a(model, train_loader, optimizer, criterion, device, hard_miner=None):
    """Training epoch with Config A optimizations"""
    model.train()
    total_loss = 0
    num_batches = 0
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        # Update hard negative miner more frequently (from tuned version)
        if hard_miner is not None and num_batches % 20 == 0:
            hard_miner.model = model
        
        # Get embeddings with 6-head attention
        ...
        # Progress indicator (less frequent due to larger batches)
        if batch_idx == 0:
            print(f"    First batch processed successfully")
        elif (batch_idx + 1) % 30 == 0:  # Less frequent due to larger batches
...
def evaluate_model_config_a(model, val_loader, device):
    """Fast evaluation"""
    model.eval()
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(val_loader):
            ...
            # Get predictions with 6-head attention
            ...
            # Progress indicator (adjusted for larger batches)
            if (batch_idx + 1) % 15 == 0:
...
def run_tier2_config_a():
    """Run Tier 2 evaluation with Config A hyperparameters"""
    print("="*80)
    print("TIER 2 ENHANCED TWO-TOWER - CONFIG A")
    print("="*80)
    print("Config A Optimizations:")
    print("  - 6 attention heads (vs 4)")
    print("  - Batch size: 64 (vs 48)")
    ...
    # CONFIG A HYPERPARAMETERS
    n_folds = 10
    epochs = 25
    batch_size = 64  # INCREASED from 48
    base_lr = 0.0008
    ...
    print(f"Expected memory usage: ~{batch_size/48:.1f}x previous (larger batches)")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(unique_queries)):
        ...
        # Create model with Config A architecture
        model = TwoTowerModel(
            num_llms=num_llms,
        ...
        for epoch in range(epochs):
            ...
            train_loss = train_epoch_config_a(model, train_loader, optimizer, criterion, device, active_miner)
            ndcg_10, ndcg_5, mrr = evaluate_model_config_a(model, val_loader, device)
    ...
    prev_best = 0.4194  # From tuned version
    improvement = np.mean(ndcg_10_scores) - prev_best
    print(f"\nComparison to previous tuned version:")
    print(f"  Previous: 0.4194 nDCG@10")
    print(f"  Config A: {np.mean(ndcg_10_scores):.4f} nDCG@10")
    ...
    results = {
        'evaluation_info': {
            ...
            'pipeline': 'Tier2 CPU Optimized (Config A)',
            'description': 'Enhanced Two-Tower with Config A: 6 attention heads, batch size 64',
    ...
    # Save results
    save_standardized_results(results, 'tier2_cpu_optimized_config_a', '../../data/results/')
    print("\n✓ Results saved to data/results/tier2_cpu_optimized_config_a_results.json")
    
    print("\n" + "="*80)
    print("Config A evaluation complete!")
    print("="*80)
