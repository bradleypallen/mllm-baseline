#!/usr/bin/env python3
"""
Tier 2 Enhanced Two-Tower - CONFIG O

CONFIG L DATASET + DEEPER LLM TOWER:
- Config L's proven conservative weak labeling dataset (501K examples)
- Enhanced LLM tower: 256D → 512D → 384D → 256D → 128D (4 layers vs 3)
- Hypothesis: Deeper LLM representations improve ranking while keeping proven data

Single Variable Experiment: Only change LLM tower depth, keep everything else identical to Config L
"""
...
class PretrainedTwoTowerModel(nn.Module):
    """CONFIG O: Config L + Deeper LLM Tower (4 layers vs 3)"""
    ...
    def __init__(self, num_llms=1131, embedding_dim=128, num_heads=4, dropout=0.2,
        ...
        # CONFIG O: DEEPER LLM TOWER (4 layers vs Config L's 3)
        self.llm_embedding = nn.Embedding(num_llms, 256)
        self.llm_tower = nn.Sequential(
            nn.Linear(256, 512),    # Layer 1: 256D → 512D
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(512, 384),    # Layer 2: 512D → 384D (NEW LAYER)
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(384, 256),    # Layer 3: 384D → 256D  
            nn.ReLU(),
            nn.Dropout(dropout),
    ...
    def encode_llms(self, llm_ids):
        """Encode LLM IDs using DEEPER tower (Config O enhancement)"""
...
def train_epoch_config_o(model, train_loader, optimizer, criterion, device, hard_miner=None):
    """Training epoch for Config O (Config L + deeper LLM tower)"""
    model.train()
    total_loss = 0
    num_batches = 0
    ...
    for batch_idx, batch in enumerate(train_loader):
        if batch_idx == 0:
            print(f"    Starting training with deeper LLM tower...")
...
def evaluate_model_config_o(model, val_loader, device):
    """Fast evaluation for Config O"""
    model.eval()
...
def run_tier2_config_o():
    """Run Tier 2 evaluation with Config O (Config L + deeper LLM tower)"""
    print("=" * 80)
    print("TIER 2 ENHANCED TWO-TOWER - CONFIG O")
    print("=" * 80)
    print("Config O - CONFIG L DATASET + DEEPER LLM TOWER:")
    print("  - Dataset: Config L's proven conservative weak labeled (501K examples)")
    print("  - Architecture: Deeper LLM tower (4 layers vs 3)")
    print("  - LLM tower: 256D → 512D → 384D → 256D → 128D")
    print("  - Query tower: Identical to Config L (proven)")
    ...
    print()
    print("Hypothesis: Deeper LLM representations improve ranking beyond Config L's 0.4471 nDCG@10")
    print()
    
    # Use GPU if available for Config O peak performance
    ...
    # CONFIG O HYPERPARAMETERS (Same as Config L - proven optimal)
    n_folds = 10
    ...
    print(f"LLM Tower: DEEPER (256→512→384→256→128) vs Config L (256→512→256→128)")
    print(f"Pre-training: {pretrained_model_path}")
    print(f"Expected: Deeper LLM representations + Config L dataset = improved performance")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(original_queries)):
        ...
        # Create model with Config O architecture (Config L + deeper LLM tower)
        ...
        # Same successful loss and optimizer as Config L
        ...
        for epoch in range(epochs):
            ...
            train_loss = train_epoch_config_o(model, train_loader, optimizer, criterion, device, active_miner)
            ndcg_10, ndcg_5, mrr = evaluate_model_config_o(model, val_loader, device)
    ...
    print("\n" + "=" * 80)
    print("FINAL RESULTS - CONFIG O (CONFIG L DATASET + DEEPER LLM TOWER)")
    print("=" * 80)
    ...
    # Comparison with Config L baseline
    config_l_ndcg = 0.4471  # Config L baseline result
    current_ndcg = np.mean(ndcg_10_scores)
    improvement = current_ndcg - config_l_ndcg
    
    print(f"\nDeeper LLM Tower Analysis:")
    print(f"  Config L (3-layer LLM tower):  {config_l_ndcg:.4f} nDCG@10")
    print(f"  Config O (4-layer LLM tower):  {current_ndcg:.4f} nDCG@10")
    print(f"  Architectural improvement:     {improvement:+.4f}")
    
    if improvement > 0.01:
        print(f"  ✅ SUCCESS: Deeper LLM tower significantly improves performance!")
        improvement_pct = improvement / config_l_ndcg * 100
        print(f"     {improvement_pct:.1f}% improvement from deeper LLM representations")
    elif improvement > 0.005:
        print(f"  📈 MODEST GAIN: Deeper LLM tower provides small but meaningful improvement")
    else:
        print(f"  📊 MIXED: Deeper LLM tower shows similar performance to 3-layer baseline")
    ...
    results = {
        ...
        'architectural_analysis': {
            ...
            'config_o_deeper_llm': float(current_ndcg),
            'architectural_improvement': float(improvement),
            'improvement_percentage': float(improvement / config_l_ndcg * 100),
    ...
    # Save results
    save_standardized_results(results, 'tier2_config_o', '../../data/results/')
    print("\n✓ Results saved to data/results/tier2_config_o_results.json")
    
    print("\n" + "=" * 80)
    print("Config O (Config L dataset + deeper LLM tower) evaluation complete!")
    print("=" * 80)
