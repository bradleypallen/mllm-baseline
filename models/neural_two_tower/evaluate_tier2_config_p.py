#!/usr/bin/env python3
"""
Tier 2 Enhanced Two-Tower - CONFIG P

512D LLM EMBEDDINGS EXPERIMENT:
- Config L's proven conservative weak labeling dataset (501K examples)
- Enhanced LLM embeddings: 512D (vs 256D in Config O)
- Same 4-layer tower depth as Config O
- Hypothesis: Larger embedding space improves LLM representation capacity

Single Variable Experiment: Only change LLM embedding dimension to 512D
"""
...
class PretrainedTwoTowerModel(nn.Module):
    """CONFIG P: 512D LLM Embeddings + 4-layer tower"""
    ...
    def __init__(self, num_llms=1131, embedding_dim=128, num_heads=4, dropout=0.2,
        ...
        # CONFIG P: 512D LLM EMBEDDINGS (doubled from 256D)
        self.llm_embedding = nn.Embedding(num_llms, 512)  # 512D instead of 256D
        self.llm_tower = nn.Sequential(
            nn.Linear(512, 768),    # Layer 1: 512D → 768D (scaled up)
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(768, 512),    # Layer 2: 768D → 512D
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(512, 256),    # Layer 3: 512D → 256D  
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(256, embedding_dim)  # Layer 4: 256D → 128D
        )
    ...
    def encode_llms(self, llm_ids):
        """Encode LLM IDs using 512D embeddings and 4-layer tower"""
...
def train_epoch_config_p(model, train_loader, optimizer, criterion, device, hard_miner=None):
    """Training epoch for Config P (512D LLM embeddings)"""
    model.train()
    total_loss = 0
    num_batches = 0
    ...
    for batch_idx, batch in enumerate(train_loader):
        if batch_idx == 0:
            print(f"    Starting training with 512D LLM embeddings...")
...
def evaluate_model_config_p(model, val_loader, device):
    """Fast evaluation for Config P"""
    model.eval()
...
def run_tier2_config_p():
    """Run Tier 2 evaluation with Config P (512D LLM embeddings)"""
    print("=" * 80)
    print("TIER 2 ENHANCED TWO-TOWER - CONFIG P")
    print("=" * 80)
    print("Config P - 512D LLM EMBEDDINGS EXPERIMENT:")
    print("  - Dataset: Config L's proven conservative weak labeled (501K examples)")
    print("  - Architecture: 512D LLM embeddings (doubled from 256D)")
    print("  - LLM tower: 512D → 768D → 512D → 256D → 128D (4 layers)")
    print("  - Query tower: Identical to Config L/O (proven)")
    ...
    print()
    print("Hypothesis: Larger embedding space improves LLM representation capacity")
    print("Expected: Better discrimination between 1,131 LLMs with richer embeddings")
    print()
    
    # Use GPU if available for Config P peak performance
    ...
    # CONFIG P HYPERPARAMETERS (Same as Config L/O - proven optimal)
    n_folds = 10
    ...
    print(f"LLM Embeddings: 512D (vs 256D in Config O)")
    print(f"LLM Tower: 512→768→512→256→128 (scaled for 512D input)")
    print(f"Pre-training: {pretrained_model_path}")
    print(f"Expected: Richer LLM representations → improved ranking performance")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(original_queries)):
        ...
        # Create model with Config P architecture (512D LLM embeddings)
        ...
        # Same successful loss and optimizer as Config L/O
        ...
        for epoch in range(epochs):
            ...
            train_loss = train_epoch_config_p(model, train_loader, optimizer, criterion, device, active_miner)
            ndcg_10, ndcg_5, mrr = evaluate_model_config_p(model, val_loader, device)
    ...
    print("\n" + "=" * 80)
    print("FINAL RESULTS - CONFIG P (512D LLM EMBEDDINGS)")
    print("=" * 80)
    ...
    # Comparison with Config O baseline
    config_o_ndcg = 0.4482  # Config O result (4-layer 256D)
    config_l_ndcg = 0.4471  # Config L baseline (3-layer 256D)
    ...
    print(f"\n512D LLM Embeddings Analysis:")
    print(f"  Config L (3-layer, 256D):  {config_l_ndcg:.4f} nDCG@10")
    print(f"  Config O (4-layer, 256D):  {config_o_ndcg:.4f} nDCG@10")
    print(f"  Config P (4-layer, 512D):  {current_ndcg:.4f} nDCG@10")
    print(f"  Improvement vs Config O:  {improvement_vs_o:+.4f}")
    print(f"  Improvement vs Config L:  {improvement_vs_l:+.4f}")
    
    if improvement_vs_o > 0.01:
        print(f"  ✅ SUCCESS: 512D embeddings significantly improve performance!")
        improvement_pct = improvement_vs_o / config_o_ndcg * 100
        print(f"     {improvement_pct:.1f}% improvement from larger embedding space")
    elif improvement_vs_o > 0.005:
        print(f"  📈 MODEST GAIN: 512D embeddings provide small but meaningful improvement")
    else:
        print(f"  📊 MIXED: 512D embeddings show similar performance to 256D baseline")
    ...
    results = {
        ...
        'architectural_analysis': {
            'config_l_baseline': config_l_ndcg,
            'config_o_256d': config_o_ndcg,
            'config_p_512d': float(current_ndcg),
            'improvement_vs_config_o': float(improvement_vs_o),
            'improvement_vs_config_l': float(improvement_vs_l),
            'improvement_percentage_vs_o': float(improvement_vs_o / config_o_ndcg * 100) if config_o_ndcg > 0 else 0.0,
    ...
    # Save results
    save_standardized_results(results, 'tier2_config_p', '../../data/results/')
    print("\n✓ Results saved to data/results/tier2_config_p_results.json")
    
    print("\n" + "=" * 80)
    print("Config P (512D LLM embeddings) evaluation complete!")
    print("=" * 80)
