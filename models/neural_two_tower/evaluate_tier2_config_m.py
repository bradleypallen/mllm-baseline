#!/usr/bin/env python3
"""
Tier 2 Enhanced Two-Tower - CONFIG M

CONFIG J + MASSIVE-SCALE WEAK LABELED DATA:
- Config J's proven 256D LLM embedding architecture
- Extended training (35 epochs, patience=15)
- MASSIVE weak labeled dataset: 500K-1M+ real discovery examples
- Ultimate test: Best architecture + maximum real data scale

Hypothesis: Massive-scale real weak labeled data achieves peak performance
"""
...
class PretrainedTwoTowerModel(nn.Module):
    ...
    def __init__(self, num_llms=1131, embedding_dim=128, num_heads=4, dropout=0.2,
        ...
        # LLM tower (CONFIG M: Same as Config J - 256D embeddings)
...
def train_epoch_config_m(model, train_loader, optimizer, criterion, device, hard_miner=None):
    """Training epoch for Config M (Config J + massive weak labeled)"""
    model.train()
    total_loss = 0
    num_batches = 0
    ...
    for batch_idx, batch in enumerate(train_loader):
        if batch_idx == 0:
            print(f"    Starting training on massive dataset...")
        ...
        elif (batch_idx + 1) % 100 == 0:  # Less frequent for massive dataset
...
def evaluate_model_config_m(model, val_loader, device):
    """Fast evaluation for Config M"""
    model.eval()
...
def run_tier2_config_m():
    """Run Tier 2 evaluation with Config M (Config J + massive weak labeled)"""
    print("=" * 80)
    print("TIER 2 ENHANCED TWO-TOWER - CONFIG M")
    print("=" * 80)
    print("Config M - MASSIVE-SCALE WEAK LABELED REAL DATA TEST:")
    print("  - Base: Config J architecture (256D LLM embeddings)")
    print("  - Extended training: 35 epochs with patience=15")
    print("  - MASSIVE weak labeled dataset: 500K-1M+ real discovery examples")
    print("  - 4 attention heads + batch size 96 (proven optimal)")
    print("  - Learning rate: 0.0008 with cosine annealing")
    print("  - Real discovery data knowledge at maximum scale")
    print()
    print("Hypothesis: Massive-scale real weak labeled data achieves PEAK performance")
    print()
    
    # Use GPU if available for Config M peak performance
    device = torch.device('mps' if torch.backends.mps.is_available() else 'cpu')
    print(f"Using device: {device}")
    if device.type == 'cpu':
        print(f"CPU threads: {torch.get_num_threads()}")
    else:
        print("GPU acceleration enabled for massive-scale performance")
    
    # Load MASSIVE weak labeled dataset
    massive_data_path = '../../data/supervised_training_with_massive_weak_labels.csv'
    
    # Check if massive dataset exists, otherwise wait
    if not os.path.exists(massive_data_path):
        print(f"⏳ Waiting for massive weak labeling to complete...")
        print(f"   Looking for: {massive_data_path}")
        print(f"   Please run weak_label_discovery_massive.py first")
        return None
    
    df = load_data(massive_data_path)
    
    print(f"MASSIVE WEAK LABELED DATASET LOADED:")
    ...
    # CONFIG M HYPERPARAMETERS (Same as Config J - proven optimal)
    n_folds = 10
    ...
    print(f"Expected: Config J architecture + massive weak labeled real data = ULTIMATE performance")
    ...
    print(f"Training includes massive weak labeled discovery data for maximum generalization")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(original_queries)):
        ...
        # Create datasets - training includes ALL data (original + massive weak labeled)
        ...
        # Training: original train queries + ALL massive weak labeled data
        if 'source' in df.columns:
            original_train_mask = df['query_id'].isin(train_queries) & (df['source'] == 'original')
            massive_weak_mask = df['source'] == 'massive_weak_labeled_discovery'
            train_mask = original_train_mask | massive_weak_mask
        ...
        print(f"Train: {len(train_df):,} examples (including massive weak labeled data)")
        ...
        # Create model with Config M architecture (Config J + massive weak labeled data)
        ...
        for epoch in range(epochs):
            ...
            train_loss = train_epoch_config_m(model, train_loader, optimizer, criterion, device, active_miner)
            ndcg_10, ndcg_5, mrr = evaluate_model_config_m(model, val_loader, device)
    ...
    print("\n" + "=" * 80)
    print("FINAL RESULTS - CONFIG M (MASSIVE-SCALE WEAK LABELED REAL DATA)")
    print("=" * 80)
    ...
    config_l_ndcg = 0.4471  # Config L weak labeled result
    current_ndcg = np.mean(ndcg_10_scores)
    improvement_vs_j = current_ndcg - config_j_ndcg
    improvement_vs_l = current_ndcg - config_l_ndcg
    
    print(f"\nMassive-Scale Weak Labeled Analysis:")
    print(f"  Config J (256D, normal scale):      {config_j_ndcg:.4f} nDCG@10")
    print(f"  Config L (256D, 114K weak labeled): {config_l_ndcg:.4f} nDCG@10") 
    print(f"  Config M (256D, massive weak):      {current_ndcg:.4f} nDCG@10")
    print(f"  Improvement vs Config J:            {improvement_vs_j:+.4f}")
    print(f"  Improvement vs Config L:            {improvement_vs_l:+.4f}")
    
    if current_ndcg > config_l_ndcg:
        print(f"  ✅ SUCCESS: Massive weak labeled data beats all previous approaches!")
        improvement_pct = improvement_vs_l / config_l_ndcg * 100
        print(f"     {improvement_pct:.1f}% improvement over Config L")
    elif improvement_vs_l > 0.005:
        print(f"  📈 MODEST GAIN: Massive scale provides small improvement")
    else:
        print(f"  📊 PLATEAU: May have reached quality vs quantity limits")
    
    # Prepare standardized results
    results = {
        'evaluation_info': {
            'timestamp': datetime.now().isoformat(),
            'pipeline': 'Tier2 Config M (Massive-Scale Weak Labeled Real Data)',
            'description': 'Config J (256D LLM embeddings) + massive weak labeled discovery data (500K-1M+ examples)',
            'dataset': 'TREC 2025 Million LLMs Track - Massive Weak Labeled Discovery',
            'total_examples': len(df),
            'unique_queries': df['query_id'].nunique(),
            'unique_llms': num_llms,
            'massive_weak_examples': len(df[df['source'] == 'massive_weak_labeled_discovery']) if 'source' in df.columns else 0,
            'original_examples': len(df[df['source'] == 'original']) if 'source' in df.columns else len(df),
            'massive_weak_ratio': len(df[df['source'] == 'massive_weak_labeled_discovery']) / len(df) if 'source' in df.columns else 0.0,
            'hyperparameters': {
                'config': 'M',
                'base_config': 'J',
                'pretrained': True,
        ...
        'massive_weak_analysis': {
            'config_j_baseline': config_j_ndcg,
            'config_l_weak_labeled': config_l_ndcg,
            'config_m_massive_weak': float(current_ndcg),
            'improvement_vs_config_j': float(improvement_vs_j),
            'improvement_vs_config_l': float(improvement_vs_l),
            'improvement_percentage': float(improvement_vs_l / config_l_ndcg * 100),
            'dataset_scale_multiplier': len(df) / 386801  # vs original dataset
        }
    }
    
    # Save results
    save_standardized_results(results, 'tier2_config_m', '../../data/results/')
    print("\n✓ Results saved to data/results/tier2_config_m_results.json")
    
    print("\n" + "=" * 80)
    print("Config M (massive-scale weak labeled real data) evaluation complete!")
    print("=" * 80)
